...
namespace TTS
{
    ...
    {        
        ...
        public state(int NumOfStreams)
        {            
            streams = new stream[NumOfStreams];            
        }
    }
    [Serializable()]
    class stream
    {
        public double[] MixtureWeights;
        public double[][] mean;
        public double[][] variance;
        public double[] GConst;
        public stream(int NumOfMix,int SizeOfVector)
        {
            GConst=new double[NumOfMix];
            MixtureWeights=new double[NumOfMix];
            mean = new double[NumOfMix][];
            variance = new double[NumOfMix][];
            for (int m = 0; m < NumOfMix; m++)
            {
                mean[m] = new double[SizeOfVector];
                variance[m] = new double[SizeOfVector];
            }
        }
    }
            ...
            while (pos != -1)
            {
                ...
                for (int i = 0; i < nummix & match.Success; i++)
                {
                    ...
                    for (int t = 0; t < variance[0].Length; t++)
                        variance[i][t] = double.Parse(temp[t + 2]);
                    match = match.NextMatch();
                }
